...
import { GlobeIcon, MailIcon, PhoneIcon, DownloadIcon, BookOpenIcon, GitForkIcon } from "lucide-react";
      ...
      <span className="text-gray-600 dark:text-[#8b949e]">{RESUME_DATA.skills.aiAndLibraries.join(", ")}</span>
            ...
            <div id="tab-navigation" className="flex items-center gap-4 border-b border-gray-200 dark:border-[#21262d] pb-2">
              <button 
                ...
                className={`flex items-center gap-1.5 text-sm font-medium pb-2 -mb-[10px] transition-colors border-b-2 ${
                  activeTab === "overview" 
              ...
              <>
                ...
                <div className="pt-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {RESUME_DATA.projects.slice(0, 4).map((project) => (
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-[#8b949e]">
                          {project.techStack.slice(0, 2).map((tech) => (
                            <span key={tech} className="flex items-center gap-1">
                              <span className="w-2.5 h-2.5 rounded-full bg-yellow-400 dark:bg-[#f1e05a]" />
                              {tech}
                            </span>
                          ))}
                        </div>
                            ...
                            {project.techStack.map((tech, i) => (
                              <span key={tech} className="flex items-center gap-1.5">
                                <span className={`w-3 h-3 rounded-full ${
                                  i % 5 === 0 ? "bg-yellow-400 dark:bg-[#f1e05a]" :
